...
export const ExpiryAlertsFromMockData: Story = {
  args: {
    showNotificationDot: true,
    notificationWidget: <StoryExpiryNotificationWidget alerts={expiringAlerts} />,
  },
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => {
      expect(canvas.getByText('Membership Expiry Alerts')).toBeInTheDocument();
      expect(canvas.getByText('Carlo Reyes')).toBeInTheDocument();
      expect(canvas.getByText('Levi Santos')).toBeInTheDocument();
      expect(canvas.queryByText('Dana Gomez')).not.toBeInTheDocument();
    });
  },
};

export const EmptyExpiryAlertsFromStoryMocks: Story = {
  args: {
    showNotificationDot: true,
    notificationWidget: (
